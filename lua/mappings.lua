...
map("n", "<A-.>", function() require("nvchad.tabufline").next() end, { desc = "Buffer 移动到前一个Buffer" })
map("n", "<A-,>", function() require("nvchad.tabufline").prev() end, { desc = "Buffer 移动到后一个Buffer" })
...
-- global lsp mappings
map("n", "gd", function() vim.lsp.buf.definition() end, { desc = "LSP definition(跳转到定义)" })
map("n", "K", function() vim.lsp.buf.hover() end, { desc = "LSP 查看签名帮助信息" })
map("n", "gi", function() vim.lsp.buf.implementation() end, { desc = "LSP 接口实现" })
map("n", "<leader>D", function() vim.lsp.buf.type_definition() end, { desc = "LSP 查看签名类型定义" })
map("n", "<leader>ra", function() require("nvchad.renamer").open() end, { desc = "LSP 修改变量名称" })
map("n", "<leader>ca", function() vim.lsp.buf.ode_action() end, { desc = "LSP 代码事件(如修复,重构等)" })
map("n", "gr", function() vim.lsp.buf.references() end, { desc = "LSP 查看何处引用" })
map("n", "<leader>lf", function() vim.diagnostic.open_float { border = "rounded" } end, { desc = "LSP 浮动框中显示诊断信息" })
map("n", "[d", function() vim.diagnostic.goto_prev { float = { border = "rounded" } } end, { desc = "LSP 跳转前个诊断信息处" })
map("n", "]d", function() vim.diagnostic.goto_next { float = { border = "rounded" } } end, { desc = "LSP 跳转下个诊断信息处" })
map("n", "<leader>ee", function() vim.diagnostic.setloclist() end, { desc = "LSP 显示所有的诊断信息" })

map("n", "<leader>wa", function() vim.lsp.buf.add_workspace_folder() end, { desc = "Workspace 添加workspace" })
map("n", "<leader>wr", function() vim.lsp.buf.remove_workspace_folder() end, { desc = "Workspace 移除workspace" })
map("n", "<leader>wl", function() print(vim.inspect(vim.lsp.buf.list_workspace_folders())) end,
  { desc = "Workspace 显示所有的workspace" })
